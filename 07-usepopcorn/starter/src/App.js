				...
				<Box>
					{selectedId ? (
						...
						<>
							...
							<WatchedMoviesList watched={watched} onDeleteWatch={handleDeleteWatched} />
						</>
					)}
				</Box>
			</Main>
...
function MovieDetails({ selectedId, onCloseMovie, onAddWatched, watched }) {
	...
	useEffect(
		function () {
			if (!title) return;
			document.title = `${title || "Movie Details"} - usePopcorn`;
					...
					<section>
						<div className="rating">
							{!isWatched ? (
								...
								<p>You rated this movie a {watchedUserRating}</p>
							)}
						</div>
						<p>
							<em>{plot}</em>
